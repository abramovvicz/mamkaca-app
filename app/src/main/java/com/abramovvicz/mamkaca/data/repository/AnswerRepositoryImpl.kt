...
class AnswerRepositoryImpl : AnswerRepository {
     ...
     */
    override suspend fun getAnswersByUserIdAndDate(userId: String, date: String): Flow<List<Answer>> = flow {
        validateUserId(userId)
        validateDate(date)
    ...
    override suspend fun saveAnswer(answer: Answer): Flow<Answer> = flow {
        ...
        try {
            ...
            val updatedAnswer = answer.copy(updatedAt = ZonedDateTime.now().toString())
            
            // Zapisz do Supabase - upsert
            val result = client.from(answersTable)
                .update(updatedAnswer) {
                    // Użyj ID jako klucza do aktualizacji
//                    onConflict("id")
                }
                .decodeSingle<Answer>()
            ...
            if (result.userId != null) {
                val userAnswers = userAnswersCache[result.userId]?.toMutableList() ?: mutableListOf()
            ...
            if (result.deviceId != null) {
                val deviceAnswers = deviceAnswersCache[result.deviceId]?.toMutableList() ?: mutableListOf()
        ...
        try {
            ...
            if (answerToDelete != null) {
                if (answerToDelete.userId != null) {
                    val userAnswers = userAnswersCache[answerToDelete.userId]?.toMutableList() ?: mutableListOf()
                    userAnswersCache[answerToDelete.userId] = userAnswers.filterNot { it.id == answerId }
                }
                
                if (answerToDelete.deviceId != null) {
                    val deviceAnswers = deviceAnswersCache[answerToDelete.deviceId]?.toMutableList() ?: mutableListOf()
                    deviceAnswersCache[answerToDelete.deviceId] = deviceAnswers.filterNot { it.id == answerId }
                }
            }
     ...
     */
    override suspend fun getAnswersByPeriod(userId: String, startDate: String, endDate: String): Flow<List<Answer>> = flow {
        validateUserId(userId)
        ...
        try {
            ...
            throw mapException(e, "Błąd podczas pobierania odpowiedzi z okresu $startDate - $endDate: ${e.message}")
        }
    }.flowOn(Dispatchers.IO)
