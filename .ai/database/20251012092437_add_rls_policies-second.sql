...
-- Purpose: Add Row Level Security policies to the database - DISABLED BY DEFAULT
-- Created: 2025-10-12
...
/*
IMPORTANT: These policies are prepared but NOT ACTIVE by default.
To activate Row Level Security:

1. First enable RLS for each table by uncommenting the RLS section in the initial schema
2. Then apply these policies 

These steps should be performed when moving to production or when 
security requirements demand RLS activation.
*/

comment on migration is 'Prepared Row Level Security policies for future use - inactive until RLS is enabled';
